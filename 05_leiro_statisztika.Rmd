...
texts <- texts %>%
  mutate(
    ...
    jobb_bal = if_else(mineln %in% miniszterelnokok, "jobb", "bal")
  )
...
```{r  tidy = TRUE, tidy.opts=list(width.cutoff=80)}
mineln_jobb <- corpus_subset(corpus_mineln, jobb_bal == "jobb")

mineln_bal <- corpus_subset(corpus_mineln, jobb_bal == "bal")
...
ggplot(corpus_df, aes(year, f_k, color = jobb_bal)) +
  geom_point(size = 2) +
  geom_line(aes(linetype = jobb_bal), size = 1) +
