# Információ-visszakeresés és információkinyerés, leíró statisztika {#leiro_stat}
...
[^1]: A beszédeket a Hungarian Comarative Agendas Project miniszterelnöki beszéd korpuszából válogattuk: <https://cap.tk.hu/vegrehajto>
...
[^2]: A `lubridate` használatának részletes leírása megtalálható az alábbi linken: <https://rawgit.com/rstudio/cheatsheets/master/lubridate.pdf>
...
texts <- texts %>%
  mutate(
    ...
    partoldal = if_else(mineln %in% miniszterelnokok, "jobb", "bal")
  )
...
```{r}
mineln_jobb <- corpus_subset(corpus_mineln, mineln %in% c("antall_jozsef", "boross_peter", 
    "orban_viktor"))

mineln_bal <- corpus_subset(corpus_mineln, mineln %in% c("horn_gyula", "gyurcsany_ferenc", 
...
```{r}
custom_stopwords <- data_stopwords_extra
...
```{r eval=FALSE}
ggplot(corpus_df, aes(year, f_k, color = partoldal)) +
  geom_point(size = 2) +
...
# b <- ggplot(corpus_df, aes(year, f_k, color = partoldal)) +
#   geom_point(size = 2) +
#   geom_line(aes(linetype = partoldal), size = 1) +
#   geom_text(aes(label = mineln), color = "black", nudge_y = 0.15) +
#   scale_x_continuous(limits = c(1988, 2020)) +
...
[^5]: [@schutzeIntroductionInformationRetrieval2008]
...
Ezután vizualizálhatjuk is a dokumentumok egymástól való távolságát egy olyan dendogram[^6] segítségével, amely megmutatja nekünk a lehetséges dokumentumpárokat.

[^6]: Olyan ábra, amely hasonlóságaik vagy különbségeik alapján csoportosított objektumok összefüggéseit mutatja meg.
...
ggdendrogram(hierarchikus_klaszter)
```
...
result_keyness <- textstat_keyness(dfm_keyness, target = "jobb")
```
