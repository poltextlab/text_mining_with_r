...
```{r fig.cap="A Wordfish skála eredménye"}
...
A párt metaadattal összehasonlíthatjuk az egy párthoz tartozó frakcióvezetők értékeit a `facet_wrap()` használatával. Figyeljünk arra hogy az `y` tengely skáláját szabadon hagyjuk a `scales` opcióval.
...
```{r fig.cap="Wordscore skálázás eredménye"}
